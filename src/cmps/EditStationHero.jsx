...
export function EditStationHero({ station, onSaveStation, onCloseEdit }) {
  ...
  const uploadImage = async (e) => {
    ...
    setLoading(true);
    const res = await fetch('	https://api.cloudinary.com/v1_1/rambeer6/image/upload', {
      method: 'POST',
      body: data,
    });
      ...
      <section className='edit-details' style={{ background: stationInfo.bcgColor }}>
        <label className='edit-img'>
          <img
            ...
            src={!stationInfo.imgUrl.length ? demoImg : stationInfo.imgUrl}
            alt='img'
          />
          <input className='hero-upload-btn' type='file' name='imgUrl' onChange={handleImage} />
        </label>
