...
function _StationHero({ user, station = {}, onSaveStation }) {
  ...
  return (
    <section className="station-hero">
      <div className="station-hero__info" onClick={() => setIsEdit(!isEdit)}>
        ...
        <div className="station-hero__text">
          ...
          <div className="total-durtion-user">
            <p className="duration__nums">{station.likedByUser ? station.likedByUser.length : '0'} likes ∙ {calcDuration(station)}</p>
      ...
      {isEdit && <EditStationHero station={station} onSaveStation={onSaveStation} onCloseEdit={onCloseEdit} />}
    </section>
  )
}
